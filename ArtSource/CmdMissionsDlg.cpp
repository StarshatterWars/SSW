...
void UCmdMissionsDlg::RebuildMissionList()
{
    ...
    UE_LOG(LogTemp, Warning,
        TEXT("[CmdMissionsDlg] RebuildMissionList: missions=%d templates=%d"),
        Missions.size(),
        CampaignPtr->GetTemplates().size());
...
void UCmdMissionsDlg::AppendNewMissionsIfAny()
{
    ...
    // Since templates are mixed into the UI, safest behavior is to rebuild
    // if main mission count changes rather than trying to append only one side.
    static int32 LastKnownMissionCount = -1;
    static int32 LastKnownTemplateBucketCount = -1;

    const int32 CurrentMissionCount = CampaignPtr->GetMissionList().size();
    const int32 CurrentTemplateBucketCount = CampaignPtr->GetTemplates().size();

    if (LastKnownMissionCount < 0 || LastKnownTemplateBucketCount < 0)
    {
        LastKnownMissionCount = CurrentMissionCount;
        LastKnownTemplateBucketCount = CurrentTemplateBucketCount;
        return;
    }

    if (CurrentMissionCount != LastKnownMissionCount ||
        CurrentTemplateBucketCount != LastKnownTemplateBucketCount)
    {
...
void UCmdMissionsDlg::ValidateSelectionStillExists()
{
    ...
    // Check template buckets too:
    if (!bFound)
    {
        const List<TemplateList>& Templates = CampaignPtr->GetTemplates();

        for (int32 i = 0; i < Templates.size() && !bFound; ++i)
        {
            TemplateList* Bucket = Templates[i];
            if (!Bucket)
            {
                continue;
            }

            for (int32 j = 0; j < Bucket->missions.size(); ++j)
            {
                MissionInfo* Info = Bucket->missions[j];
                if (Info && Info->id == SelectedId)
                {
                    bFound = true;
                    break;
...
void UCmdMissionsDlg::AddMissionInfoToList(MissionInfo* Info, const FString& StatusOverride)
{
    if (!MissionList || !Info)
    {
        return;
    ...
    if (M)
    {
        ...
        if (LegacyPlayer && M->GetType() == Mission::TRAINING)
        {
            bTrained = LegacyPlayer->HasTrained(M->GetIdentity());
        }

        Item->MissionType = (M->GetType() == Mission::TRAINING && bTrained)
            ? TEXT("Training")
            : UTF8_TO_TCHAR(M->GetTypeName());

        Item->MissionStatus = StatusOverride.IsEmpty() ? TEXT("Active") : StatusOverride;
    }
    else
    {
        switch (Info->type)
        ...
        Item->MissionStatus = StatusOverride.IsEmpty() ? TEXT("Available") : StatusOverride;
    }
...
void UCmdMissionsDlg::AddTemplateMissionsToList()
{
    if (!CampaignPtr)
    {
        return;
    ...
    UE_LOG(LogTemp, Warning,
        TEXT("[CmdMissionsDlg] AddTemplateMissionsToList: templates=%d"),
        Templates.size());

    for (int32 i = 0; i < Templates.size(); ++i)
    {
        TemplateList* Bucket = Templates[i];
        if (!Bucket)
        {
        ...
        UE_LOG(LogTemp, Warning,
            TEXT("[CmdMissionsDlg] Template bucket[%d]: missions=%d"),
            i,
