            ...
            if (action->IsAvailable()) {
                ...
                case ECombatActionType::COMBAT_EVENT:
                {
                    CombatEvent* event = new CombatEvent(
                        campaign,
                        static_cast<ECombatEventType>(action->Subtype()),
...
CampaignPlanEvent::CreateEvent(CombatAssignment* a)
{
    ...
    if (campaign && a && a->GetResource() && RandomChance(1, 2)) {
        event_time = FMath::RandRange(event_time, (int)campaign->GetTime());

        CombatGroup* group = a->GetResource();

        if (group == campaign->GetPlayerGroup()) {

            if (group->GetType() == ECOMBATGROUP_TYPE::DESTROYER_SQUADRON ||
                group->GetType() == ECOMBATGROUP_TYPE::BATTLE_GROUP ||
                group->GetType() == ECOMBATGROUP_TYPE::CARRIER_GROUP) {
        ...
        if (target && target == campaign->GetPlayerGroup()) {
            ...
            if (target->GetType() == ECOMBATGROUP_TYPE::DESTROYER_SQUADRON ||
                target->GetType() == ECOMBATGROUP_TYPE::BATTLE_GROUP ||
                target->GetType() == ECOMBATGROUP_TYPE::CARRIER_GROUP) {
...
CampaignPlanEvent::ChooseAssignment(CombatGroup* g)
{
    ...
    if (alist.size() > 0) {
        while (tries-- > 0) {
            ...
            int index = FMath::RandRange(0, alist.size() - 1);
...
CampaignPlanEvent::CreateEventFighterStrike(CombatAssignment* a)
{
    ...
    info += GetTeamName(group);
    info += Text("/navy? ") + group->GetDescription();
