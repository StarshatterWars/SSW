...
void
PowerSource::AddClient(SimSystem* client)
{
	if (client) {
...
void
PowerSource::RemoveClient(SimSystem* client)
{
...
PowerSource::ExecFrame(double seconds)
{
	...
	// fuel leak?
	if (availability < 0.4 && capacity > 0.0)
	...
	ListIter<SimSystem> iter = clients;
	while (++iter) {
		SimSystem* sink = iter.value();
