...
HangarSlot::HangarSlot()
	: ship(0), group(0), iff(0), design(0), deck(0), slot(0),
...
{
	FMemory::Memzero(nslots, sizeof(nslots));
	FMemory::Memzero(squadrons, sizeof(squadrons));
}
	...
	int* def_load, int maint_count, int dead_count)
{
	if (nsquadrons < MAX_SQUADRONS && count > 0) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (def_load)
				FMemory::Memcpy(s[i].loadout, def_load, sizeof(s[i].loadout));
		}
		...
		while (maint_count-- > 0) {
			s[i].state = MAINT;
			// was: 600 + rand()/15
			s[i--].time = 600.0 + (double)FMath::RandRange(0, 2184); // approx range of rand()/15
		}
...
bool
Hangar::GotoActiveFlight(int squadron, int slot_index, SimElement* elem, int* loadout)
{
	if (elem && squadron < nsquadrons && slot_index < nslots[squadron]) {
		...
		if (slot->state == STORAGE) {
			...
			if (loadout)
				FMemory::Memcpy(slot->loadout, loadout, sizeof(slot->loadout));

			Sim* sim = Sim::GetSim();

			Text ship_name = slot->squadron;

			// was: ship->Location() + RandomPoint()
			// Replace with a small random spawn offset in world space:
...
bool
Hangar::GotoAlert(int squadron, int slot, FlightDeck* d, SimElement* elem, int* loadout, bool pkg, bool expedite)
{
	if (squadron < nsquadrons && slot < nslots[squadron]) {
		...
		if (s->state == STORAGE) {
			...
			if (loadout)
				FMemory::Memcpy(s->loadout, loadout, sizeof(s->loadout));
...
int
Hangar::GetActiveElements(List<SimElement>& active_list)
{
	active_list.clear();
