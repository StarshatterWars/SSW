...
MissionTemplate::MissionTemplate(int32 Id, const FString& Filename, const FString& Path)
	: Mission(
		Id,
		...
		if (pdef) {
			...
			if (defname == "objective") {
				...
				if (!pdef->term() || !pdef->term()->isStruct()) {
					...
					TermStruct* sval = pdef->term()->isStruct();
					Instruction* npt = ParseInstruction(sval, elem);
			...
			if (defname == "min") {
				...
				if (select) {
					if (!pdef->term() || !pdef->term()->isStruct()) {
						...
						TermStruct* oval = pdef->term()->isStruct();
						if (ParseOptional(oval))
							n++;
						else
							skip++;
					}
