...
public:
	...
	FVector           attachments[MAX_STORES];   // attachment points on the rail
	int               nstores;                   // number of stores on this hard point
	FVector           eject;                     // eject velocity from rail in 3D
	...
	int               turret_axis;      // 0=az  1=el  2=notxsupported
	...
	float             damage;           // if beam, damage per second; else, damage per shot.
